...
*   V3.4  04-Apr-11, M Liu      Ignored the char case in EPOCH_STYLE field.
*   V3.5  14-Feb-17, M Liu      Use the maximum length of variable's name and
*                               FORMAT attribute for the text output.
*
******************************************************************************/
...
void SaveSettings () {
  ...
  for (Item = itemHead; Item != NULL; Item = Item->nextItem) {
	 ...
	 if (!simpleMode) {
	   ...
	   fprintf (fp, ",");
	   fprintf (fp, "%d,", MAXIMUM(Item->width,(int)strlen(Item->Var->name)));
