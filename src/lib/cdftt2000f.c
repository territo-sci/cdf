...
Fif_GHOSTDEF(string_len)
{
  ...
  *tt2000 = CDF_TT2000_from_UTC_string (DESCRtoREFnul(string,TT2000_3_STRING_LEN,&ssh));
#endif
#if defined(Fif_GHOSTLEN)
  *tt2000 = CDF_TT2000_from_UTC_string (NULterminate(string,Fif_GHOSTUSE(string_len),&ssh));
#endif
#if defined(Fif_NOLEN)
  *tt2000 = CDF_TT2000_from_UTC_string (FindEndNUL(string,TT2000_3_STRING_LEN,&ssh));
#endif
  FreeStrings (ssh);
  return;
}
