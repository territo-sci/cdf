...
switch (Va->item) {
  ...
  ****************************************************************************/
  ...
  case zVAR_BLOCKINGFACTOR_: {
    ...
    Int32 recSize, numDims, dimSizes[CDF_MAX_DIMS], dimVarys[CDF_MAX_DIMS], 
          numElems, dataType;
    int i;
    SelectCDF (Cur->cdf, CDF)
    if (BADzOP(CDF,!zOp)) return ILLEGAL_IN_zMODE;
    ...
    * Make sure that the blocking factor is at least the size of default for
    * any compressed variable. Otherwise, don't change/set it.
    **************************************************************************/
    if (!sX(ReadVDR64(CDF,CDF->fp,Var->VDRoffset64,Var->zVar,
		      VDR_FLAGS,&flags,
		      VDR_DIMVARYS,dimVarys,
		      VDR_DATATYPE,&dataType,
		      VDR_NUMELEMS,&numElems,
		      VDR_NULL),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
    ...
    if (zOp) {
      if (!sX(ReadVDR64(CDF,CDF->fp,Var->VDRoffset64,Var->zVar,
			VDR_zNUMDIMS,&numDims,
			VDR_zDIMSIZES,dimSizes,
			VDR_NULL),&pStatus)) {
        AbortAccess64 (CDF, UPDATE, noDELETE);
        return pStatus;
      }
    ...
    if (VARcompressionBITset(flags) && RECvaryBITset(flags) && (blocking != 0)) {
      recSize = numElems;
      recSize *= (Int32) CDFelemSize(dataType);
      for (i = 0; i < (int) numDims; ++i) if (dimVarys[i]) recSize *= dimSizes[i];
      if (blocking < (Int32) (MIN_BLOCKING_BYTES_compressed/recSize)) {
	sX (BLOCKINGFACTOR_TOO_SMALL2, &pStatus);
/*	return pStatus; */
      }
    }
