	                      ...
	                      Logical debug)
{
  ...
  if (VXR.VXRnext > 0) {
    ...
    if (status != CDF_OK) return status;
      ...
      return QuitCDF ("CDF(VXR): a link offset to next rec is negative in a VIR: ",
...
static CDFstatus ValidateUIR (vFILE *fp, Int32 offset, Logical debug)
{
  ...
  if (UIR.RecordSize < UIR_BASE_SIZE) 
    return QuitCDF ("CDF(UIR): record size is invalid for Unused Internal Record",
                        offset,
                    4, 1, &(UIR.RecordSize), 0, debug);
  if (UIR.NextUIR < 0) 
    return QuitCDF ("CDF(UIR): offset to next UIR is invalid for Unused Internal Record",
                        offset,
                    4, 1, &(UIR.NextUIR), 0, debug);
  if (UIR.PrevUIR < 0) 
    return QuitCDF ("CDF(UIR): offset to previous UIR is invalid for Unused Internal Record",
                        offset,
